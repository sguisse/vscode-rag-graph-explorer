...
# Update webview/src/features/exporter/components/tabs/prompt/hooks/use-prompt-tab.ts
cat << 'EOF' > webview/src/features/exporter/components/tabs/prompt/hooks/use-prompt-tab.ts
...
echo "✅ Fixed Prompt Tab state persistence: store.config.prompt is now the single source of truth."
