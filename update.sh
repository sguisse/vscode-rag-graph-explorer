...
# Overwrite ExplorerTabContainer.tsx with reference-guarded synchronization loops
...
# Bump bundle compilation targets to 1.0.7
sed -i.bak 's/"version": "[0-9]*\.[0-9]*\.[0-9]*"/"version": "1.0.7"/' package.json
rm -f package.json.bak

# Compile everything via production configurations
npm run package

echo "✅ Success: Solved the asynchronous state sync race condition! Graph and Tree single clicks are now perfectly persistent!"
