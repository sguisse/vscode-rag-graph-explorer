...
echo "🚀 Updating Codebase Explorer, Graph, and Files Context with extracted handlers, clipboard copy, and ViewMode auto-collapse..."
...
# 3. Update use-codebase-explorer-panel.ts with viewMode change auto-collapse logic
...
# 4. Update CodebaseExplorerPanel.tsx (single click -> reveal + copy, double click -> reveal + open)
...
# 5. Update CenterPanelContainer.tsx (single click -> revealInExplorer + copyToClipboard, double click -> revealInExplorer + openFile)
...
# 6. Update Cytoscape instance with single click reveal + copy to clipboard & double click open file
