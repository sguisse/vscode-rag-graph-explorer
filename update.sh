...
# Update PythonScriptExecution.manager.ts to support custom process timeouts per execution
cat << 'EOF' > backend/src/managers/PythonScriptExecution.manager.ts
'use strict';
...
echo "✅ updated: PythonScriptExecution.manager.ts now accepts an optional timeout override in executeScript!"
