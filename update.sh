...
# 1. Create UI Models & Interfaces file: model-ui.ts
...
# 2. Create Constants file: constants/codebase-explorer.constants.ts
cat << 'EOF' > "${BASE_DIR}/constants/codebase-explorer.constants.ts"
...
# 3. Create Tree Utilities file: utils/codebase-tree.utils.ts
cat << 'EOF' > "${BASE_DIR}/utils/codebase-tree.utils.ts"
...
# 4. Create TriStateCheckbox component: components/TriStateCheckbox.tsx
cat << 'EOF' > "${BASE_DIR}/components/TriStateCheckbox.tsx"
...
# 5. Create RecursiveFolderNode component: components/RecursiveFolderNode.tsx
cat << 'EOF' > "${BASE_DIR}/components/RecursiveFolderNode.tsx"
...
# 6. Refactor custom hook: hooks/use-codebase-explorer-panel.ts
cat << 'EOF' > "${BASE_DIR}/hooks/use-codebase-explorer-panel.ts"
...
# 7. Update Finder hook: hooks/use-treeview-finder.ts
cat << 'EOF' > "${BASE_DIR}/hooks/use-treeview-finder.ts"
...
# 8. Main CodebaseExplorerPanel Component File: CodebaseExplorerPanel.tsx
...
echo "✅ refactor: Extracted UI models to model-ui.ts, tree utilities to codebase-tree.utils.ts, exported interaction handlers in hook, and separated subcomponents into dedicated files with zero regression!"
