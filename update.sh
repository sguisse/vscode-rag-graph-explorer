...
# ==============================================================================
# Token Razor - Unified SidebarLeft Rendering Refactoring
# Unifies SDLC_WORKFLOW_STEPS and SIDEBAR_MENU_ITEMS rendering pipeline
# to support isHeader, children, 10px padding, and accurate step highlighting.
# ==============================================================================

echo "🎨 Updating SidebarLeft.tsx to use unified rendering for all menu items..."

cat << 'EOF' > webview/src/components/app/layout/SidebarLeft.tsx
...
export const SDLC_WORKFLOW_STEPS: NavItem[] = [
  {
    ...
    children: [
      ...
      { id: 'RESULTS_MANAGER', icon: Terminal, label: '4. Results Manager' },
    ],
  },
  { id: 'CONFIGURATION', icon: Settings, label: 'Configuration', bottom: true },
  { id: 'feature-help', icon: HelpCircle, label: 'Help & Shortcuts', bottom: true },
...
export function renderHeaderGroupItem(
  ...
  return (
    <div key={item.id} className="space-y-1 my-1">
      {/* Group Header Row */}
      {sidebarLeftMode === 'normal' ? (
        <div className="flex items-center justify-between px-2.5 py-1.5 font-mono font-bold text-[11px] text-muted-foreground uppercase tracking-wider select-none">
...
echo "✅ Successfully unified SidebarLeft menu rendering for all groups and items!"
