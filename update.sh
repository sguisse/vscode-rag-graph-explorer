...
# Ensure workflow hooks directory exists
mkdir -p webview/src/components/app/workflow/hooks

# Update use-workflow-panel.ts to use 'preset' layout in Cytoscape
cat << 'EOF' > webview/src/components/app/workflow/hooks/use-workflow-panel.ts
...
echo "✅ feat/fix: Configured Cytoscape layout to 'preset' to render nodes using exact (x, y) coordinates from workflowData!"
