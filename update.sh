...
FINDER_HOOK="webview/src/features/explorer/wkp-lft-codebase-tree/hooks/use-treeview-finder.ts"

mkdir -p "webview/src/components/app"
mkdir -p "webview/src/features/explorer/wkp-lft-codebase-tree/hooks"

# 1. Update TopMiddleBottomPanel with strict flexbox slot isolation
cat << 'EOF' > "${PANEL_UTIL}"
...
# 2. Update use-treeview-finder.ts to replace scrollIntoView with container-bounded scrollTo
cat << 'EOF' > "${FINDER_HOOK}"
...
# 3. Update CodebaseExplorerPanel.tsx with strict bounds
...
echo "✅ fix: Replaced window-level scrollIntoView with container-isolated scrollTo logic!"
