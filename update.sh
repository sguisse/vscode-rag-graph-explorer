...
echo "🚀 Updating single-click file/folder selection to reveal in VS Code Explorer without opening files..."
...
# 1. Update CodebaseExplorerPanel.tsx
...
function RecursiveFolderNode({
  ...
  const handleFolderClick = () => {
    ...
    if (node.folderPath) {
      logInfo(`Folder clicked: ${node.folderPath}. Selecting in VS Code Explorer...`);
      vsCodeApiService.revealInExplorer(node.folderPath);
    }
  };

  const handleFileClick = (file: CodebaseFile) => {
    if (file.path) {
      logInfo(`File clicked: ${file.path}. Selecting in VS Code Explorer...`);
...
export function GraphPanel({
  ...
  const handleNodeClick = useCallback((node: GraphNodeData) => {
    const targetPath = node.path || node.absolutePath || node.filePath;
    if (targetPath) {
      logInfo(`Graph node clicked: ${node.id} (${targetPath}). Selecting in VS Code Explorer...`);
      vsCodeApiService.revealInExplorer(targetPath);
    }
    if (onSelectEntity) {
      onSelectEntity({ type: 'node', nodeId: node.id });
    }
  }, [onSelectEntity]);

  return (
    <div id="panel-graph-workspace" className={`flex flex-col bg-background h-full w-full relative ${className}`}>
      <div className="flex-1 p-4 overflow-auto">
        <div className="grid grid-cols-1 gap-2">
          {nodes.map((node) => {
            const isSelected = selectedEntity?.nodeId === node.id;
            const nodePath = node.path || node.absolutePath || node.filePath;

            return (
              <div
                key={node.id}
                className={`p-2.5 rounded border transition-colors cursor-pointer flex items-center justify-between font-mono text-xs ${
                  isSelected
              ...
              >
                <div className="flex flex-col min-w-0 flex-1 pr-2">
                  ...
                  {nodePath && (
                    <span className="text-[10px] text-muted-foreground truncate">{nodePath}</span>
                  )}
                </div>
              </div>
            );
...
# 3. Update files-context.tsx
cat << 'EOF' > webview/src/features/explorer/wkp-rgt-tabs-files-context/files-context.tsx
import React, { useCallback } from 'react';
import { vsCodeApiService } from '@/services/api/vs-code-api.service.gen';
import { logInfo } from '@/services/view/log-view.service.wrapper';
import { CodebaseFile, SelectedEntity } from '@/shared/services/graph-rag-explorer';
...
export function FilesContextTab({
  files = [],
  selectedEntity,
  onSelectEntity,
  className = ''
  ...
  const handleFileItemClick = useCallback((file: CodebaseFile) => {
    if (file.path) {
      logInfo(`FilesContext item clicked: ${file.id} (${file.path}). Selecting in VS Code Explorer...`);
      vsCodeApiService.revealInExplorer(file.path);
    }
    if (onSelectEntity) {
      onSelectEntity({ type: 'node', nodeId: file.id });
    }
  }, [onSelectEntity]);

  return (
    <div id="panel-files-context" className={`flex flex-col bg-card h-full w-full font-mono text-xs ${className}`}>
          ...
          return (
            <div
              ...
              className={`flex items-center gap-2 p-2 rounded border transition-colors cursor-pointer ${
                isSelected
                  ? 'bg-primary/10 border-primary text-primary font-bold'
                  : 'bg-muted/20 hover:bg-muted border-border text-foreground'
              }`}
...
echo "✅ feat: Configured single-click file and folder selection to invoke revealInExplorer without opening text files!"
