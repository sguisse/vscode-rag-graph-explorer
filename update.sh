...
# Target path
PANEL_DIR="webview/src/features/sdlc/domains/instructions/bmad-method/components"
PANEL_FILE="${PANEL_DIR}/BMadMethodPanel.tsx"

# Ensure target directory exists
mkdir -p "${PANEL_DIR}"

# Write updated BMadMethodPanel component using TopMiddleBottomPanel and toolbar controls
cat << 'EOF' > "${PANEL_FILE}"
...
export function BMadMethodPanel() {
  ...
  const topContent = (
    <div className="flex justify-between items-center bg-muted/20 p-1 border-border border-b shrink-0 font-mono text-xs w-full">
      <div className="flex items-center gap-1.5 pl-1.5 min-w-0">
        <Sparkles size={14} className="text-indigo-400 shrink-0" />
        <span className="font-bold text-[11px] text-indigo-400 uppercase tracking-wider truncate">
...
# Rebuild webview application
cd webview && npm run build

echo "✅ feat: Added top toolbar with Expand All and Collapse All buttons and integrated TopMiddleBottomPanel layout in BMadMethodPanel!"
