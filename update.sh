...
# Update files-ctx-export-panel.tsx to add padding (p-1) inside the scrollable container so focus halos are not clipped
cat << 'EOF' > webview/src/features/explorer/components/files-ctx-export/files-ctx-export-panel.tsx
...
echo "✅ style: Fixed focus halo clipping on Max Chunk input field by adding container padding (p-1)!"
