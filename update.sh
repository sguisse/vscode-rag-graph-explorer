#!/bin/bash
# Production-ready repair script to fix the invalid shutil attribute in the jQAssistant worker

# Ensure target directory exists
mkdir -p scripts/analyser/tools/java/jqassistant

# Provide the complete corrected worker script with shutil.copy2
...
class JQAssistantWorker(BaseAnalyser):
    ...
    def run_analysis(self, manifest_data: dict, neo4j_client: Neo4jClient, config_matrix: dict) -> None:
        ...
        # CRITICAL ASSURANCE RESYNC: Ensure the runtime workspace has the configuration file copied
        if os.path.exists(custom_config_path) and not os.path.exists(os.path.join(jqa_run_dir, ".jqassistant.yml")):
            shutil.copy2(custom_config_path, os.path.join(jqa_run_dir, ".jqassistant.yml"))

        custom_env = os.environ.copy()

        # 3. Diagnostics Phase
        self._dump_diagnostics(executable_target, jqa_run_dir, custom_env)

        # 4. Execution Phase (Scan & Analyze via custom configurations discovered via active working directory context)
    ...
    def _dump_diagnostics(self, executable_target: str, jqa_run_dir: str, custom_env: dict) -> None:
        reports_dir = os.path.join(jqa_run_dir, "../../install_reports/java_jqassistant")
...
echo "✅ fix: Corrected the invalid 'shutil.copy4' reference to 'shutil.copy2' within the jQAssistant analytical worker node."
