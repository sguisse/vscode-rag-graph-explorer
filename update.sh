...
TARGET_DIR="webview/src/features/sdlc/domains/instructions/bmad-method"

mkdir -p "${TARGET_DIR}/components"
mkdir -p "${TARGET_DIR}/model"

# Update BMadInstructionsPanel.tsx
cat << 'EOF' > "${TARGET_DIR}/components/BMadInstructionsPanel.tsx"
...
echo "✅ feat: Added auto-copy checkbox before domain dropdowns, auto-initialized 1st domain/subdomain, and fixed line-feed formatting for textareas!"
