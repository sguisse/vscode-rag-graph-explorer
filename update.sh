...
def build_statistics(neo4j_client, workspace_root: str):
    ...
    try:
        # En supprimant database=target_db, le driver Python tape automatiquement dans la base de données principale (neo4j).
        with neo4j_client.driver.session() as session:

            # DIAGNOSTIC RAPIDE : On loggue le total brut pour vérifier que la session Python voit bien les mêmes noeuds que le navigateur.
                    ...
                    for entry in entries:
                        ...
                        if not loops:
                            try:
                                ...
                                # CONVERSION SAFE : On force en dictionnaire Python pour éviter les bugs de l'objet Record Neo4j
                                result_val = run_res.data().get("n", 0) if run_res else 0
