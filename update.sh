...
# 1. Overwrite TreeView.tsx to bind single-clicks (files/folders) to native VS Code sidebar reveal hooks
...
# 2. Bump package runtime distribution token context to 1.0.8
sed -i.bak 's/"version": "[0-9]*\.[0-9]*\.[0-9]*"/"version": "1.0.8"/' package.json
rm -f package.json.bak

# 3. Rebuild the application targets
npm run package

echo "✅ Success: Coupled unified single-clicks to selection overlays inside VS Code native explorer layout!"
