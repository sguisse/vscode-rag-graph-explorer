...
# ── Load skill.env via the package bootstrap helper
from .bootstrap import load_skill_env_vars
...
def venv_install(
    ...
    print(f"\n{GREEN}🐍  Python environment installation -----------------------------------------{RESET}")
...
def main() -> int:
    ...
    status, msg, python_bin = venv_install(py_prj_root, args.venv_dir, args.force)

    icon = format_status(status)
    print(f"{icon} {msg}")
    ...
    if status == "ERROR":
        return 2
    if status == "SKIP":  # venv exists and was skipped (--force not set)
        return 1
