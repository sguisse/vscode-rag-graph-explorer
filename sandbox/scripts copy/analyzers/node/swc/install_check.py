...
class SWCChecker:
    ...
    def check_swc(self):
        ...
        if os.path.exists(os.path.join(node_dir, "node_modules", "@swc", "core")):
            swc_ok = True
        ...
        if swc_ok:
            ...
            self.status["swc"] = {
                "status": "❌",
                "message": "The @swc/core native parsing library package is missing from local node analyzer layout contexts."
            }
            self.ko_count += 1
...
def run_check():
    ...
    info(json.dumps(checker.status, ensure_ascii=False), component="SWCCheck")
