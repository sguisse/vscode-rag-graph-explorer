#!/bin/bash
# Critical hotfix compilation repair script resolving the trailing tag syntax error in the Inspector container
...
export default function App() {
  ...
  const [config] = useState({ tooltipDelay: 200 });
  ...
  return (
    <div id="ctn-root" className={`flex flex-col h-screen w-screen overflow-hidden font-sans text-sm select-none transition-colors duration-200 bg-background text-foreground ${isDarkMode ? 'dark' : ''}`}>
      ...
      <LayoutPanel
        ...
        right={
          <div className="flex items-center gap-1">
            <Button id="btn-import-dialog" variant="ghost" size="icon" onClick={() => setImportOpen(true)} className="hover:bg-muted p-1.5 rounded text-muted-foreground hover:text-foreground transition-colors w-8 h-8" data-tooltip="Import local AST JSON/YAML schema payload extracts"><Upload size={16} /></Button>
            <Button id="btn-export-dialog" variant="ghost" size="icon" onClick={() => setExportOpen(true)} className="hover:bg-muted p-1.5 rounded text-muted-foreground hover:text-foreground transition-colors w-8 h-8" data-tooltip="Export current topological session structure"><Download size={16} /></Button>

            <div className="mx-1 bg-border w-px h-4"></div>

            <Button id="btn-toggle-theme" variant="ghost" size="icon" onClick={() => setIsDarkMode(!isDarkMode)} className="hover:bg-muted p-1.5 rounded text-muted-foreground hover:text-foreground transition-colors w-8 h-8" data-tooltip={isDarkMode ? "Switch to crisp light mode theme" : "Switch to immersive dark mode theme"}>
              {isDarkMode ? <Sun size={16} /> : <Moon size={16} />}
            </Button>

            <Button id="btn-reset-graphe" variant="ghost" size="icon" onClick={resetAllFilters} className="hover:bg-muted p-1.5 rounded text-muted-foreground hover:text-foreground transition-colors w-8 h-8" data-tooltip="Reset all workspace visual states, filters, and matrices"><RotateCcw size={16} /></Button>
              ...
              <ResizableContainer id="ctn-workspace-right" visible={isCtnWorkspaceRightVisible} style={{ width: !isCtnWorkspaceCenterVisible ? '100%' : `${ctnWorkspaceRightWidth}%` }} headerLeft="Metadata & Inspector Tab Matrices" className={!isCtnWorkspaceCenterVisible ? 'flex-1 border-l min-w-[200px]' : 'border-l min-w-[200px]'} resizeHandle={isCtnWorkspaceCenterVisible ? "left" : "none"} onResizeStart={isCtnWorkspaceCenterVisible ? startCtnWorkspaceRightResize : undefined}>
                <div className="flex flex-col bg-card h-full">
                  <div className="flex bg-muted/40 border-border border-b shrink-0">
                    <Button id="btn-inspector-tab-inspect" variant="ghost" onClick={() => setRightPanelTab('inspect')} className={`flex-1 py-2 text-center font-mono text-[11px] font-bold h-auto rounded-none border-b-2 ${rightPanelTab === 'inspect' ? 'border-primary text-primary bg-background' : 'text-muted-foreground border-transparent'}`} data-tooltip="Inspect detailed structural properties, docs, and impact records of selection">Inspector</Button>
                    <Button id="btn-inspector-tab-plantuml" variant="ghost" onClick={() => setRightPanelTab('plantuml')} className={`flex-1 py-2 text-center font-mono text-[11px] font-bold h-auto rounded-none border-b-2 ${rightPanelTab === 'plantuml' ? 'border-primary text-primary bg-background' : 'text-muted-foreground border-transparent'}`} data-tooltip="Generate and copy standard PlantUML class diagram code models">PlantUML</Button>
                    <Button id="btn-inspector-tab-jsonschema" variant="ghost" onClick={() => setRightPanelTab('json_schema')} className={`flex-1 py-2 text-center font-mono text-[11px] font-bold h-auto rounded-none border-b-2 ${rightPanelTab === 'json_schema' ? 'border-primary text-primary bg-background' : 'text-muted-foreground border-transparent'}`} data-tooltip="View data specification guidelines for the system dependency parser">JSON Schema</Button>
                  </div>
                  <div className="flex-1 p-4 overflow-y-auto font-mono text-xs">
                            ...
                            <div className="space-y-4 animate-in duration-200 fade-in">
                              ...
                              <div className="space-y-3 bg-orange-500/5 p-4 border border-orange-500/25 rounded-lg">
                                <div className="flex justify-between items-center">
                                  ...
                                  <Button id="btn-copy-impact-recipes" onClick={() => copyToClipboard(generatedMarkdownRecipe, "Markdown impact recipe copied to clip-board!")} className="flex items-center gap-1 bg-muted hover:bg-muted/80 px-2 py-1 border border-border rounded font-mono text-[10px] text-foreground h-6" data-tooltip="Copy actionable test verification plan checklist to clipboard">
      ...
      <div
        ...
        className="fixed hidden z-50 pointer-events-none bg-slate-900/95 dark:bg-slate-950/95 text-slate-100 px-2.5 py-1.5 rounded-md shadow-xl border border-slate-800 font-sans text-[11px] font-medium leading-normal max-w-xs break-words"
        style={{ display: 'none' }}
      />
...
# Execute Vite production build script to verify compiler sanity locally
if [ -f "package.json" ]; then
  npm run build
fi

echo "✅ fix/compile: Corrected the unclosed tag typo within the directional impact panel and successfully generated a error-free production-ready compilation tree!"
