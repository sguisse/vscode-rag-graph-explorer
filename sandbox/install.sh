...
mkdir -p src/features/explorer/wkp-bottom-infos

# 1. Create the hook service for context paths
cat << 'EOF' > src/features/explorer/wkp-top-paths/use-context-paths.ts
...
# 2. Create the context paths panel component
cat << 'EOF' > src/features/explorer/wkp-top-paths/context-paths-panel.tsx
...
# 3. Create the bottom panel component
cat << 'EOF' > src/features/explorer/wkp-bottom-infos/wkp-bottom-panel.tsx
import React from 'react';
...
# 4. Update ExplorerFeature.tsx to incorporate both top and bottom externalized panels
...
import { WkpBottomPanel } from './wkp-bottom-infos/wkp-bottom-panel';
...
echo "✅ refactor: Externalized top and bottom context layout components to dedicated panels and hook layers! Run 'npm run build' to recompile."
