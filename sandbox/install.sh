...
# ✅ feat: Embed react-resizable-panels into ResizableContainer and refactor App.tsx

mkdir -p src/components/app

# 1. Update ResizableContainer
...
export interface ResizableContainerProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  separatorInnerClassName?: string;
...
export function ResizableContainer({
  ...
  visible = true,
  type = 'container',
  defaultSize, minSize, maxSize, order,
  orientation, onLayoutChanged,
  withSeparator, separatorId, separatorClassName, separatorInnerClassName,
  headerLeft, headerCenter, headerRight,
  titleBarId, contentId, headerClassName, contentClassName,
  ...
  if (type === 'group') {
    return (
      <Group id={id} orientation={orientation || 'horizontal'} onLayout={onLayoutChanged} className={className} style={style}>
        {children}
      </Group>
    );
  }
  ...
  if (type === 'panel') {
    return (
      <>
        <Panel id={id} defaultSize={defaultSize} minSize={minSize} maxSize={maxSize} order={order} style={style}>
          {innerContent}
        </Panel>
        {withSeparator && (
          <Separator id={separatorId || `${id}-handle`} className={separatorClassName}>
            {separatorInnerClassName && <div className={separatorInnerClassName} />}
          </Separator>
        )}
      </>
    );
...
const hInnerHandle = "bg-border/50 rounded-full w-[2px] h-8";
const vInnerHandle = "bg-border/50 rounded-full w-8 h-[2px]";
    ...
    <TooltipProvider>
      <div id="ctn-root" className={`flex flex-col h-screen w-screen overflow-hidden font-sans text-sm select-none transition-colors duration-200 bg-background text-foreground ${isDarkMode ? 'dark' : ''}`}>
        ...
        <div id="ctn-main" className="relative flex flex-1 overflow-hidden">
          <ResizableContainer type="group" id="main-horizontal-layout" orientation="horizontal" onLayoutChanged={handleGraphLayoutResize}>

            {/* B. SIDEBAR LEFT */}
            <ResizableContainer
              type="panel"
              id="panel-sidebar-left"
              defaultSize={250} minSize={5} maxSize={600} order={1}
              visible={sidebarLeftMode !== 'collapsed'}
              withSeparator={isCtnWorkspaceVisible || isSidebarRightVisible}
              separatorId="ctn-sidebar-left-handle"
              separatorClassName={hResizeHandleClass}
              separatorInnerClassName={hInnerHandle}
            ...
            <ResizableContainer
              type="panel"
              id="panel-workspace-stage"
              order={2} defaultSize={65}
              visible={isCtnWorkspaceVisible}
              withSeparator={isSidebarRightVisible}
              separatorId="ctn-sidebar-right-handle"
              separatorClassName={hResizeHandleClass}
              separatorInnerClassName={hInnerHandle}
            ...
            >
              <div id="ctn-workspace" className="relative flex flex-col flex-1 min-w-0 h-full">
                <div id="ctn-workspace-wrapper-lvl-1" className="relative flex flex-col flex-1 min-w-0 h-full">
                  <ResizableContainer type="group" id="workspace-vertical-layout" orientation="vertical" onLayoutChanged={handleGraphLayoutResize}>

                    {/* TOP COLLAPSIBLE CONTAINER */}
                    <ResizableContainer
                      type="panel"
                      id="panel-workspace-top"
                      defaultSize={215} minSize={5} maxSize={600} order={1}
                      visible={isCtnWorkspaceTopVisible}
                      withSeparator={true}
                      separatorId="ctn-workspace-top-handle"
                      separatorClassName={vResizeHandleClass}
                      separatorInnerClassName={vInnerHandle}
                      headerLeft="Selected paths"
                      className="bg-muted border-b h-full"
                    >
                      <div className="p-1">
                        <ul id="paths-list" className="space-y-1.5 p-2 text-muted-foreground text-xs">
                    ...
                    <ResizableContainer
                      type="panel"
                      id="panel-workspace-middle-row"
                      order={2} defaultSize={70}
                      withSeparator={isCtnWorkspaceBottomVisible}
                      separatorId="ctn-workspace-bottom-handle"
                      separatorClassName={vResizeHandleClass}
                      separatorInnerClassName={vInnerHandle}
                      className="bg-transparent border-none"
                    >
                      <div id="ctn-workspace-middle-row" className="flex flex-1 w-full h-full min-h-0 overflow-hidden">
                        <ResizableContainer type="group" id="middle-row-horizontal-layout" orientation="horizontal" onLayoutChanged={handleGraphLayoutResize}>

                          {/* LEFT TIER CONTAINER */}
                          <ResizableContainer
                            type="panel"
                            id="panel-workspace-left-tier"
                            defaultSize={220} minSize={15} maxSize={400} order={1}
                            visible={isCtnWorkspaceLeftVisible}
                            withSeparator={isCtnWorkspaceCenterVisible || isGraphMaximized || isCtnWorkspaceRightVisible}
                            separatorId="ctn-workspace-left-handle"
                            separatorClassName={hResizeHandleClass}
                            separatorInnerClassName={hInnerHandle}
                            headerLeft={getActiveViewLabel()}
                            className="border-r min-w-[200px] h-full"
                          >
                            <div className="flex flex-col justify-between h-full">
                              <div className="flex-1 bg-background overflow-auto scrollbar-hide">
                                {renderViewContent()}
                              </div>
                              <ResizableContainer type="container" id="panel-logs" headerLeft="Parser Logs" className="border-t h-[140px]" titleBarId="panel-logs-title-bar">
                          ...
                          <ResizableContainer
                            ...
                            order={2}
                            defaultSize={isCtnWorkspaceLeftVisible && isCtnWorkspaceRightVisible ? 50 : 70}
                            minSize={20}
                            ...
                            withSeparator={isCtnWorkspaceRightVisible}
                            separatorId="ctn-workspace-center-handle"
                            separatorClassName={hResizeHandleClass}
                            separatorInnerClassName={hInnerHandle}
                          ...
                          <ResizableContainer
                            type="panel"
                            id="panel-workspace-right-tier"
                            defaultSize={220} minSize={10} maxSize={400} order={3}
                            visible={isCtnWorkspaceRightVisible}
                            headerLeft="Workspace Right"
                            className="border-l min-w-[200px] h-full"
                            titleBarId="ctn-workspace-right-title-bar"
                          >
                    ...
                    <ResizableContainer
                      type="panel"
                      id="panel-workspace-bottom-pane"
                      defaultSize={30} minSize={5} maxSize={600} order={3}
                      visible={isCtnWorkspaceBottomVisible}
                      className="bg-secondary border-t h-full"
                    >
                      <LayoutPanel
            ...
            <ResizableContainer
              type="panel"
              id="panel-sidebar-right-inspector"
              defaultSize={200} minSize={15} maxSize={450} order={3}
                ...
                {selectedIds.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    ...
                    {selectedIds.map(id => {
                      ...
                      if(!node) return null;
                      return (
                        <div key={id} className="bg-background border border-border rounded-md overflow-hidden">
                          <div className="flex justify-between items-center bg-secondary px-2.5 py-1.5 border-border border-b">
                            <span className="font-semibold text-foreground">{node.data.label}</span>
                            <span className="bg-primary/10 px-1 py-0.5 border border-primary/20 rounded font-bold text-[9px] text-primary uppercase">{node.data.layer}</span>
                          </div>
                          <div className="space-y-1 p-2 text-[11px] text-muted-foreground">
                             ...
                             <div className="flex justify-between"><span>Incoming:</span> <span className="font-bold text-destructive-foreground">{AST_DATA.edges.filter(e => e.data.target === id).length}</span></div>
                             <div className="flex justify-between"><span>Outgoing:</span> <span className="font-bold text-primary">{AST_DATA.edges.filter(e => e.data.source === id).length}</span></div>
                          </div>
                        </div>
                      )
                    })}
...
echo "✅ refactor: Embedded react-resizable-panels v4 Group, Panel, Separator into ResizableContainer smoothly!"
