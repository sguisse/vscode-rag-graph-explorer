...
# 2. Update CodebaseExplorerPanel.tsx: Replace header import button with download/export AST button
cat << 'EOF' > src/features/explorer/wkp-lft-codebase-tree/CodebaseExplorerPanel.tsx
import React, { useRef, useEffect } from 'react';
import { ChevronDown, ChevronRight, Folder, FileCode, Database, Download } from 'lucide-react';
...
export function CodebaseExplorerPanel({
  ...
  return (
    <div id="panel-codebase-explorer" className="flex flex-col bg-card h-full">
      {/* Top Header with Download AST Button */}
      <div className="flex justify-end items-center bg-muted/20 p-1 px-2 border-border border-b">
        <Button
          ...
          data-tooltip="Export current AST Codebase to JSON file"
        >
          <Download size={13} />
        </Button>
      </div>

      {/* Codebase Explorer Tree */}
...
echo "✅ refactor: Replaced top header import button with AST export download button in CodebaseExplorerPanel!"
