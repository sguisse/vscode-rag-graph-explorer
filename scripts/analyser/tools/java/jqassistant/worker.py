...
class JQAssistantWorker(BaseAnalyser):
    ...
    def run_analysis(self, manifest_data: dict, neo4j_client: Neo4jClient, config_matrix: dict) -> None:
        ...
        custom_env = self._prepare_environment(custom_config_path, rules_dir)

        # 4. Diagnostics Phase (Using jqa_run_dir as CWD)
        self._dump_diagnostics(executable_target, jqa_run_dir, rules_dir, workspace_root, custom_env)

        # 5. Execution Phase (Scan & Analyze in isolated CWD)
        scan_return_code = self._execute_scan_and_analyze(
            executable_target, jqa_run_dir, discovered_sources, rules_dir, custom_env
        )
    ...
    def _prepare_environment(self, custom_config_path: str, rules_dir: str) -> dict:
        """Prepares environment variables by binding SmallRye config location and explicit rules directory."""
    ...
    def _dump_diagnostics(self, executable_target: str, jqa_run_dir: str, rules_dir: str, workspace_root: str, custom_env: dict) -> None:
        """Orchestrates the dumping of diagnostic information to dedicated log files."""
        ...
        try:
            self._dump_effective_configuration(executable_target, jqa_run_dir, rules_dir, reports_dir, custom_env)
            self._dump_available_rules(executable_target, jqa_run_dir, rules_dir, reports_dir, custom_env)
        except Exception as e:
            error(f"Failed to execute effective diagnostic commands: {e}", component=self.name)

    def _dump_effective_configuration(self, executable_target: str, jqa_run_dir: str, rules_dir: str, reports_dir: str, custom_env: dict) -> None:
    ...
    def _dump_available_rules(self, executable_target: str, jqa_run_dir: str, rules_dir: str, reports_dir: str, custom_env: dict) -> None:
    ...
    def _execute_scan_and_analyze(self, executable_target: str, jqa_run_dir: str, discovered_sources: dict, rules_dir: str, custom_env: dict) -> int:
        """Orchestrates the jQAssistant scan and analyze phases within the isolated CWD."""
        scan_return_code = self._execute_scan(executable_target, jqa_run_dir, discovered_sources, rules_dir, custom_env)

        if scan_return_code == 0:
            self._execute_analyze(executable_target, jqa_run_dir, rules_dir, custom_env)

        return scan_return_code

    def _execute_scan(self, executable_target: str, jqa_run_dir: str, discovered_sources: dict, rules_dir: str, custom_env: dict) -> int:
    ...
    def _execute_analyze(self, executable_target: str, jqa_run_dir: str, rules_dir: str, custom_env: dict) -> int:
