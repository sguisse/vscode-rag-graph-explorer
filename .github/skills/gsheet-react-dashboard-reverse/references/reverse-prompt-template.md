Apply this Prompt on canvas named `GSHEET_DASHBORD_SHEET_NAME`:
...
1. Reconstruct an observed Google Sheets Canvas dashboard (`GSHEET_DASHBORD_SHEET_NAME`) into a formal technical specification (`CANVAS_IR`).
2. Generate an implementation spec for a React reproduction (`<FeatureNameClone/>`) that embeds mandatory operational upgrades into its Header (`<SpecsTrigger/>` and `<CellOriginsToggle/>`).
...
├── features/
│   ├── <feature-name>/          # Main feature components (<FeatureName>Header, <FeatureName>Grid, VirtualTable, KPI, Cards)
...
- **Component Stack & Security:**
  ...
  - `<SpecsModal/>`: Base UI Dialog portal overlay (`role="dialog"`, `aria-modal="true"`, Z-index: 50). Includes search bar and tab deep-linking.
...
- **Component Stack & Portals:**
  - `<CellOriginsToggle/>`: Checkbox injected into `<FeatureNameHeader/>` (pure in-memory toggle state).
  - `<OriginTag/>`: Wrapper component surrounding source-backed UI fields.
  - `<OriginTooltip/>`: Rendered inside a **React Portal** via Base UI / Floating UI (Z-index: 60) to prevent clipping in `overflow: hidden` parent containers or virtualized table rows.
...
2. The `<FeatureNameHeader/>` layout code showing injection of `<CellOriginsToggle/>` and `<SpecsTrigger/>`.
...
> *"Part 1 Blueprint complete. Reply 'CONTINUE' to generate the 7-Module Embedded Markdown Document."*
...
> **"REVERSE COMPILER READY. Send your source dashboard inputs (Screenshots, DOM tree, HTML/CSS, CSV Data, or Formulas) to begin Stage 1 Reverse Engineering."**
