...
description: Generate the reverse-engineering prompt template from references/reverse-prompt-<feature-name>.template.md to extract technical specifications (CANVAS_IR) from Google Sheets dashboards.
license: MIT
metadata:
  version: "1.4.0"
  author: sguisse
...
📁 `.github/skills/gsheet-react-dashboard-reverse/references/reverse-prompt-<feature-name>.template.md`
...
- The user requests a reverse-engineering prompt to analyze a Google Sheets Canvas dashboard or sheet named `<GSHEET_DASHBOARD_SHEET_NAME>`.
...
1. **Load Reference Template**: Read `.github/skills/gsheet-react-dashboard-reverse/references/reverse-prompt-<feature-name>.template.md`.
...
- **STRICT FULL OUTPUT**: The agent MUST load and output the complete content of `references/reverse-prompt-<feature-name>.template.md` without omitting any sections or using placeholders (`// TODO`, `...`, `/* rest of prompt */`).
